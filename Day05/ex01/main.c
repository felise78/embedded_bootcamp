...
void updateLEDs() {
     ...
     PORTB = (compteur[iterator] & 0b00000111); // Utilise les 3 bits de poids faible de 'value' pour les LEDs D1, D2 et D3
//    // Allumer la LED D4
    if (compteur[iterator] >= 8) {
        PORTB |= (1 << LED4_PIN);
    } else {
        // Éteindre la LED D4
        PORTB &= ~(1 << LED4_PIN);
	...
	// PORTB = (compteur[iterator] & 0x07) | ((compteur[iterator] & 0x08) << 1)
}
...
int main () {
	...
	addr[2] = (uint8_t *)0x42;
	addr[3] = (uint8_t *)0x43;
	uint8_t *moni = (uint8_t *)0x39;


 	// if ()
	for (int i = 0; i < 4; i++)
		compteur[i] = eeprom_read_byte(addr[i]); 
	iterator = eeprom_read_byte(moni); 
	while(1) {
		updateLEDs();
		if (!(PIND & (1 << SW1_PIN))) { // Si le bouton est enfoncé
			...
			if (!(PIND & (1 << SW1_PIN))) { // Vérifier à nouveau si le bouton est enfoncé
				compteur[iterator]++; // Incrémenter le compteur[iterator]
				if (compteur[iterator] == 16) // Limiter le compteur[iterator] à 4
					compteur[iterator] = 0;
				eeprom_write_byte(addr[iterator], compteur[iterator]);
		...
		if (!(PIND & (1 << 4)))
		{
			...
			if (!(PIND & (1 << 4)))
			{
				...
				if (iterator == 4)
					iterator = 0;
				eeprom_write_byte(moni, iterator);
				_delay_ms(200); // Attendre un court instant pour éviter les rebonds
				while (!(PIND & (1 << 4)));
			}
		}
